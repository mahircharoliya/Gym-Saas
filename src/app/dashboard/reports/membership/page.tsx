...
export default function MembershipAnalyticsPage() {
  ...
  const fetch_ = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/analytics/membership?range=${range}`, {
  ...
  const daily = (data?.dailySeries as { date: string; newMemberships: number }[]) ?? [];
  const plans = (lifetime?.planBreakdown as { name: string; count: number }[]) ?? [];
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatCard label="Most Popular Plan" value={String(lifetime?.mostPopularPlan ?? "—")}
            icon={Award} iconColor="text-yellow-400" iconBg="bg-yellow-500/10" />
          <StatCard label="Churn Rate" value={rangeData ? `${rangeData.churnRate}%` : "—"}
            sub={`${rangeData?.cancelledInRange ?? 0} cancelled`}
            icon={TrendingDown} iconColor="text-red-400" iconBg="bg-red-500/10" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-800 bg-white p-6">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={daily.map((d) => ({ ...d, date: fmtDate(d.date) }))} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="newMemberships" stroke="#6366f1" strokeWidth={2} fill="url(#nmGrad)" name="New Memberships" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="rounded-xl border border-gray-800 bg-white p-6">
          ...
          {plans.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={plans} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={75} innerRadius={40} paddingAngle={3}>
                  {plans.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                </Pie>
