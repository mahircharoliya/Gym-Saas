...
import { Plus, ClipboardList, Pencil, Trash2, ToggleLeft, ToggleRight, QrCode } from "lucide-react";

import QRModal from "@/components/ui/QRModal";

interface Waiver { id: string; title: string }
...
export default function FormsPage() {
    ...
    const [qrForm, setQrForm] = useState<{ slug: string; name: string } | null>(null);
    ...
    function openCreate() {
        ...
        setSelectedProducts([]); setSelectedWaivers([]);
    ...
    async function save() {
        ...
        try {
            ...
            const res = await fetch(url, {
                ...
                body: JSON.stringify({ name, description, productIds: selectedProducts, waiverIds: selectedWaivers }),
            });
        ...
        return (
            <div className="max-w-2xl space-y-5">
                <div className="flex items-center justify-between">
                    <h2 className="text-lg font-semibold text-white">{editing ? "Edit Form" : "New Signup Form"}</h2>
                    <button onClick={() => setCreating(false)} className="text-sm text-gray-400 hover:text-white">← Back</button>
                </div>
                {error && <p className="text-sm text-red-400">{error}</p>}

                <Input label="Form Name" placeholder="New Member Signup" value={name} onChange={(e) => setName(e.target.value)} />
                ...
                {/* Products */}
                <div>
                    <p className="text-sm font-medium text-gray-300 mb-2">Products to offer</p>
                    <div className="space-y-2">
                        ...
                        {products.length === 0 && <p className="text-xs text-gray-500">No products yet. Create products first.</p>}
                    </div>
                </div>

                {/* Waivers */}
                <div>
                    <p className="text-sm font-medium text-gray-300 mb-2">Waivers to include</p>
                    <div className="space-y-2">
                        {waivers.map((w) => (
                            <label key={w.id} className="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" checked={selectedWaivers.includes(w.id)}
                                    onChange={() => toggleItem(w.id, selectedWaivers, setSelectedWaivers)}
                                    className="accent-indigo-500" />
                                <span className="text-sm text-gray-300">{w.title}</span>
                            </label>
                        ))}
                        {waivers.length === 0 && <p className="text-xs text-gray-500">No waivers yet.</p>}
                    </div>
                </div>
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-0.5">Create forms with products and waivers.</p>
                </div>
            ...
            {forms.length === 0 ? (
                ...
                <div className="space-y-3">
                    {forms.map((f) => (
                        <div key={f.id} className="rounded-xl border border-gray-800 bg-gray-900 px-5 py-4">
                            <div className="flex items-start justify-between gap-4">
                                <div className="min-w-0">
                                    ...
                                    <div className="flex gap-3 mt-1.5 text-xs text-gray-500">
                                        <span>{f.productIds.length} product{f.productIds.length !== 1 ? "s" : ""}</span>
                                        <span>{f.waivers.length} waiver{f.waivers.length !== 1 ? "s" : ""}</span>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 shrink-0">
                                    <button onClick={() => toggleActive(f)} title={f.isActive ? "Deactivate" : "Activate"}
                                        className={f.isActive ? "text-emerald-400" : "text-gray-600"}>
                                        {f.isActive ? <ToggleRight size={22} /> : <ToggleLeft size={22} />}
                                    </button>
                                    <button onClick={() => setQrForm({ slug: f.slug, name: f.name })}
                                        className="text-gray-400 hover:text-indigo-400 transition-colors" title="QR Code">
                                        <QrCode size={15} />
                                    </button>
                                    <button onClick={() => openEdit(f)} className="text-gray-400 hover:text-white transition-colors">
                                        <Pencil size={15} />
                                    </button>
                                    <button onClick={() => deleteForm(f.id)} className="text-gray-400 hover:text-red-400 transition-colors">
                                        <Trash2 size={15} />
                                    </button>
                                </div>
                            </div>
