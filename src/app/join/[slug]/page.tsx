    ...
    if (notFound) {
        return (
            <div className="flex min-h-screen items-center justify-center text-center px-4">
                <div>
                    <p className="text-2xl font-bold text-black mb-2">Form not found</p>
        ...
        <div className="min-h-screen bg-gray-950 px-4 py-12">
            <div className="mx-auto max-w-lg">
                ...
                <div className="mb-8 text-center">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-600 mx-auto mb-3">
                        <Dumbbell size={20} className="text-black" />
                    </div>
                    <h1 className="text-xl font-bold text-black">{data.tenant.name}</h1>
                            ...
                            <button key={p.id} onClick={() => setSelectedProduct(p.id)}
                                ...
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="text-right shrink-0 ml-4">
                                        <p className="font-bold text-black">${Number(p.price).toFixed(2)}</p>
                ...
                {/* Step: Personal info */}
                {step === "info" && (
                    <div className="space-y-4 rounded-2xl border border-gray-800 bg-white p-6">
                        <p className="text-sm font-medium text-black">Your information</p>
                ...
                {step === "waiver" && (
                    <div className="space-y-4">
                        <p className="text-sm font-medium text-black text-center">Review & sign waivers</p>
                        {error && <p className="text-sm text-red-400 text-center">{error}</p>}
                        {data.waivers.map((w) => (
                            <div key={w.id} className="rounded-xl border border-gray-800 bg-white p-5">
                                <p className="font-medium text-black mb-3">{w.title}</p>
                                ...
                                <label className="flex items-center gap-3 cursor-pointer">
                                    <input type="checkbox" checked={acceptedWaivers.includes(w.id)}
                                        onChange={() => toggleWaiver(w.id)} className="accent-blue-500 h-4 w-4" />
